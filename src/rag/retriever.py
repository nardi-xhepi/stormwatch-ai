...
class TemporalRetriever:
    ...
    def retrieve_current_conditions(self, location: str = None) -> List[Dict[str, Any]]:
        """Retrieve latest weather for a city - simple filter, no vector search."""
        try:
            filter_conditions = [
            ...
            results = self.qdrant.scroll(
                collection_name=self.COLLECTION_NAME,
                scroll_filter=Filter(must=filter_conditions),
                limit=50,  # Get enough to find recent ones
                with_payload=True
            )[0]
            ...
            # Return top 2 as formatted dicts
            documents = []
    ...
    def retrieve_air_quality(self, location: str = None) -> List[Dict[str, Any]]:
        """Retrieve latest air quality for a city - simple filter, no vector search."""
        try:
            # Build filter
            filter_conditions = [
            ...
            # Get entries and sort by timestamp client-side
            results = self.qdrant.scroll(
                collection_name=self.COLLECTION_NAME,
                scroll_filter=Filter(must=filter_conditions),
                limit=50,
