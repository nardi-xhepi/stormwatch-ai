...
\begin{enumerate}[leftmargin=2em, itemsep=0.2em]
    ...
    \item \textbf{Embedding} -- Generate 384-dimensional vectors via \texttt{all-MiniLM-L6-v2}
...
\subsection{Embedding and Similarity Computation}

The system uses the \texttt{all-MiniLM-L6-v2} sentence transformer, balancing three considerations:

\begin{itemize}[leftmargin=2em, itemsep=0.2em]
    \item \textbf{Dimension Efficiency:} 384 dimensions (vs. 768+ for larger models)
    \item \textbf{Inference Speed:} Real-time processing without GPU
    \item \textbf{Semantic Quality:} Competitive STS benchmark performance
\end{itemize}

L2-normalization simplifies similarity computation:
\[
\mathbf{v}_t = \frac{\text{MiniLM}(t)}{\|\text{MiniLM}(t)\|} \in \mathbb{R}^{384}, \quad \|\mathbf{v}_t\| = 1
\]

\subsection{Temporal Weighting with Exponential Decay}

Rather than hard timestamp filtering which fails when no recent data exists, StormWatch AI applies soft exponential decay:

\begin{mathbox}
\[
s_{\text{final}} = s_{\text{sim}} \cdot e^{-\lambda \cdot t / t_{\max}}
\]

\vspace{0.3em}
\small
where $t$ = observation age in minutes, $t_{\max}$ = 60 minutes, $\lambda$ = 0.1
\end{mathbox}

\begin{table}[H]
\centering
\caption{Score retention over time}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{cc}
\toprule
\textbf{Age} & \textbf{Score Retention} \\
\midrule
5 minutes & 99.2\% \\
30 minutes & 95.1\% \\
1 hour & 90.5\% \\
...
\subsection{Data Schemas and Structures}

\begin{table}[H]
\centering
\caption{Data type characteristics and retrieval methods}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l c l l}
\toprule
\textbf{Type} & \textbf{Poll Freq.} & \textbf{Retrieval} & \textbf{Notes} \\
\midrule
current\_weather & 30s & Filter/sort & Latest only \\
air\_quality & 30s & Filter/sort & Latest only \\
forecast & 5 min & Timestamp sort & 2-3 entries \\
alert & Real-time & Validity check & start $\leq$ now $\leq$ end \\
news & 30s & Semantic + decay & Includes links \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{highlightbox}[Key Contribution: Differentiated Retrieval]
Rather than applying uniform RAG to all data types, the system uses:
\begin{itemize}[leftmargin=1.5em, itemsep=0.1em, topsep=0.2em]
    \item \textbf{Database-style queries} for structured weather data
    \item \textbf{Validity-period checking} for alerts
    \item \textbf{Semantic search with time decay} for news articles
\end{itemize}
This matches retrieval methods to data characteristics.
\end{highlightbox}
...
A potential extension would be to leverage LLM tool-use capabilities, where the model receives data schemas and dynamically generates Qdrant queries based on user intent. This would increase flexibility but adds latency and complexity; our explicit retrieval logic prioritizes reliability and speed for the well-defined weather domain.
