...
async def get_air_quality(city: str):
    ...
    try:
        ...
        # Use Qdrant scroll with filter - same approach as weather
        from qdrant_client.models import Filter, FieldCondition, MatchValue
        
        results = retriever.qdrant.scroll(
            collection_name=retriever.COLLECTION_NAME,
            scroll_filter=Filter(must=[
                FieldCondition(key="data_type", match=MatchValue(value="air_quality")),
                FieldCondition(key="location", match=MatchValue(value=city))
            ]),
            limit=20,
            with_payload=True
        )[0]
        ...
        if results:
            ...
            raw_data = sorted_results[0].payload.get("raw_data", {})
            return {
                ...
                "city": city,
                "timestamp": sorted_results[0].payload.get("timestamp", "")
            }
        else:
            return {
                "aqi": 0,
...
async def get_stats():
    ...
    try:
        ...
        return {
            ...
            "is_mock": weather_client.use_mock
        }
    except Exception:
        return {"online": False, "vectors": 0, "is_mock": True}
